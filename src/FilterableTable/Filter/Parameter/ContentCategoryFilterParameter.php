...
final class ContentCategoryFilterParameter implements FilterParameterInterface, ExpressionBuilderInterface
{
    ...
    {
        return [
            'label' => 'controller.document.list.filter.contentCategory',
            'attr' => [
                'class' => '',
    ...
    public function buildWhereExpression(QueryBuilder $queryBuilder, $formData, string $entityAlias): ?string
    {
        ...
        $queryBuilder
            ...
            ->leftJoin(
                $contentElementAlias.'.contentCategories',
                $contentCategoryAlias = $this->createAlias()
            )
        ;

        return (string) $queryBuilder->expr()->in($contentCategoryAlias.'.id', $ids);
    }
    ...
    {
        return $this->aliasFactory->createAlias(static::class, 'contentCategory');
    }
}
