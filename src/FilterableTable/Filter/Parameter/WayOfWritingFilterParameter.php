...
final class WayOfWritingFilterParameter implements FilterParameterInterface, ExpressionBuilderInterface
{
    ...
    {
        return [
            'label' => 'controller.document.list.filter.wayOfWriting',
            'attr' => [
                'class' => '',
                'data-vyfony-filterable-table-filter-parameter' => true,
            ],
            'class' => WayOfWriting::class,
            'choice_label' => 'name',
            'expanded' => false,
            'multiple' => true,
    ...
    public function buildWhereExpression(QueryBuilder $queryBuilder, $formData, string $entityAlias): ?string
    {
        ...
        $queryBuilder
            ->innerJoin(
                $entityAlias.'.wayOfWriting',
                $wayOfWritingAlias = $this->createAlias()
            )
        ;

        return (string) $queryBuilder->expr()->in($wayOfWritingAlias.'.id', $ids);
    }
    ...
    {
        return $this->aliasFactory->createAlias(static::class, 'wayOfWriting');
    }
}
