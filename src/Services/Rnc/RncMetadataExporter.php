...
final class RncMetadataExporter implements RncMetadataExporterInterface
{
    ...
    {
        return array_map(
            fn (Document $document) => $this->getRow($document, $baseUrl),
            $this->documentRepository->findAllInConventionalOrder($onlyShownOnSite)
        );
    }

    private function getRow(Document $document, string $baseUrl): array
    {
        return [
